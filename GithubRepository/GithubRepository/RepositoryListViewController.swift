...
class RepositoryListViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let refeshControl = self.refreshControl
        refeshControl?.backgroundColor = .white
        refeshControl?.tintColor = .darkGray
        refeshControl?.attributedTitle = NSAttributedString(string: "당겨서 새로고침")
        refeshControl?.addTarget(self, action: #selector(refresh), for: .valueChanged)
    ...
    func fetchRepositories(of organization: String) {
        Observable.from([organization]).map { organization -> URL in
